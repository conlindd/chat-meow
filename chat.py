    ...
    while True:
        ...
        headers = {
           "Content-Type": "application/json",
            "Authorization": "Bearer sk-9jxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxRN"
        }
        data = {
            "model": "gpt-3.5-turbo",
        ...
        openai_output = response_json["choices"][0]["message"]["content"]
        print(openai_output)
        ...
        # # 百度语音解析出来的文字发送给ChatGpts
        # json_data = {"message": result_text}
        # json_str = json.dumps(json_data, ensure_ascii=False)
        # code, openai_output = openai_handler.chat(json_str)
        # logging.info('openai接收到的是什么?=====+++++++++++++++++:{}'.format(code))
        # logging.info('openai接收到的是什么?=====+++++++++++++++++:{}'.format(openai_output))
        openai_lock.release()

        msg_lock.acquire()
        # 百度将收到chatGpt返回的文字生成语音
